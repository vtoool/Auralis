...
const ThemeToggle: React.FC = () => {
  ...
  return (
    <button
      ...
      className="p-2 rounded-full text-primary hover:bg-primary/10 dark:text-accent dark:hover:bg-accent/10 transition-colors duration-300"
      aria-label="Toggle theme variant"
    >
      {themeVariant === 'light' ? <MoonIcon /> : <SunIcon />}
    </button>
  );
};
