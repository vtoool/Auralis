...
const OasisShop: React.FC = () => {
    ...
    const handleBack = (e: React.MouseEvent<HTMLAnchorElement>) => {
        e.preventDefault();
        // Use history.back() to return to the previous page with scroll position preserved.
        if (window.history.length > 1) {
            window.history.back();
        } else {
            // Fallback for direct navigation or new tabs
                ...
                <div className="mb-8 pt-8">
                 ...
                 <AnimatedSection>
                    <div className="text-center mb-12 pt-12">
                        <h1 className="font-sans text-4xl font-bold text-primary tracking-widest uppercase">{t('oasis.shop.title')}</h1>
                        <p className="text-text-secondary text-lg mt-2">{t('oasis.shop.subtitle')}</p>
                    </div>
                </AnimatedSection>
